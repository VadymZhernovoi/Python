...
from .parm_const import (PAGE_SIZE, COL_YEAR_MAX, MONGO_COLS, TOP_QUERIES, COL_SEPARATOR,
                         COL_KEYWORD_MONGO, KEY_RETURN, TXT_RETURN, KEY_EXIT, TXT_EXIT, MSG_EXIT, Q_EXIT,
                         Q_SEARCH_CATEGORY, ERR_SEARCH_CATEGORY, AVAIL_VALUE, ERR_FORMAT_INP, ERR_UNKNOWN,
                         MSG_NOT_FOUND, SEARCH_TYPE, COL_CATEGORY, COL_CATEGORY_MONGO, COL_YEAR_START_MONGO,
                         COL_YEAR_STOP_MONGO, COLOR_SELECT, COLOR_RESET, COL_FILM, COLOR_TABLE_COLS, COL_YEAR_MIN)
...
def print_error(msg: str, before: bool = False):
    ...
    if before:
        print()
    console.print(f"[red]Fehler![/red] [yellow]{msg}[/yellow]")
...
def input_category(categories):
    while True:
        string = ''
        try:
            msg = f"{Q_SEARCH_CATEGORY} ({TXT_RETURN} / {TXT_EXIT})"
            ...
            if not string:
                print_error(ERR_SEARCH_CATEGORY)
                continue
            ...
            if string.isdigit():
                ...
                if 1 <= num <= len(categories):
                    ...
                    print_error(f"Das Genre mit der Nummer {string} wurde in der Liste nicht gefunden.")
            else:
                return list(filter(lambda d: d[COL_CATEGORY].upper() == string.upper(), categories))[0]

        except IndexError:
            print_error(f"Das Genre '{string}' wurde in der Liste nicht gefunden.")
            continue

def input_year_range(search_parm, year_min, year_max):
    while True:
...
def input_year(txt: str, year_min: int, year_max: int) -> int | None:
    ...
    msg = f"{AVAIL_VALUE} [{year_min}] - [{year_max}] ([Enter] - {year_enter} / {TXT_RETURN} / {TXT_EXIT})"
    ...
    try:
        ...
        if string.isdigit():
            ...
            if year_min <= year <= year_max:
                ...
                raise ValueError(f"{AVAIL_VALUE} {year_min} - {year_max}")
        else:
            if string != KEY_EXIT[0]:
                raise ValueError(f"{ERR_FORMAT_INP} '{string}'")
...
def show_statistics(tabl_first: list[dict], tabl_second: list[dict] = ''):
    ...
    if not (is_tabl_first or is_tabl_second):
        print_color(f"[{MSG_NOT_FOUND}]", "yellow")
...
def display_page_by_page(select_col: str, select_body: str, parm_find: tuple, search: str = '', keyword: str=''):
    ...
    if total_rows == 0:
        print_color(f"[{MSG_NOT_FOUND}]", "yellow", False, False)
    ...
    while outer:
        ...
        print_color(f"Filme gefunden: [{total_rows}]   (Gesamtseitenzahl: [{total_pages}])",
                    "cyan", True, False)
        ...
        if page_num < total_pages:
            msg = "[Enter] nächste / " + msg
        if page_num > 1:
            msg = "[-] vorherige / " + msg

        while True:
            nav = input_color(f"Navigation: {msg}", "cyan", False).lower()
            print()
            check_for_exit(nav)
            if nav == KEY_EXIT[0]:
                continue
            ...
            if nav == KEY_RETURN[0]:
                ...
                if offset >= PAGE_SIZE:
                    ...
                    print_color("[⏮] Sie befinden sich auf der ersten Seite.",
                                "red", False, False)
                ...
                if offset + PAGE_SIZE < total_rows:
                    ...
                    print_color("[⏭] Das ist die letzte Seite.", "red", False, False)
                ...
                if 1 <= page <= total_pages:
                    ...
                    print_color(f"⛔ Die Seite [{page}] existiert nicht (verfügbar [1]-[{total_pages}])",
                                "red", False, False)

                    continue

            else:
                print("⛔ Unbekannter Befehl. Verwenden Sie nur verfügbare Befehle!")
